...
from .ema_engine import update_sma, is_bearish, is_bearish_confirmed, is_index_bearish, reset_sma_for_symbols
    ...
    while True:
        ...
        for _, row in universe_df.iterrows():
            ...
            try:
                ...
                # ── Entry: Nifty downtrend + confirmed crossover (0.15% separation)
                #          + portfolio not stopped + slot available ─────────────
                if (
                    not portfolio_stopped
                    and is_index_bearish()
                    and is_bearish_confirmed(symbol)
        ...
        if now.hour > 15 or (now.hour == 15 and now.minute >= 15):
            logger.info("DS RUN STOP: 3:15 PM reached — closing all remaining positions.")
            for sym in list(active_signals.keys()):
                ...
                logger.info("DS CLOSE EXIT | %s @ %.2f | pnl=%.2f (%.2f%%)", sym, ltp, real_pnl, pnl_pct)
            ...
            logger.info("DS RUN STOP: Final state published. total_pnl=%.2f portfolio_pct=%.2f%%", final_real_pnl, final_portfolio_pct)
            return
