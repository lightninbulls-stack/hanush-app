...
class AlphaBearPut:
    ...
    def quote_details(self) -> None:
        log_and_print("QD 1: building local NIFTY PE chain")
        self.expiry = str(self.cred["i_expiry_date_nifty"])

        df_pe = build_local_nifty_chain_with_ltp(
            kite=self.kite,
    ...
    def start(self) -> None:
        try:
            ...
            log_and_print("STEP 1: waiting for NIFTY_BEAR_PUT_LOCK")

            with NIFTY_BEAR_PUT_LOCK:
                log_and_print("STEP 2: acquired NIFTY_BEAR_PUT_LOCK")
            ...
            log_and_print("STEP 7: released NIFTY_BEAR_PUT_LOCK")
...
# ─────────────────────────────────────────────────────────────────────────────
# EMA CROSSOVER  (NIFTY spot signal detection — bearish: EMA5 crosses below EMA55)
# ─────────────────────────────────────────────────────────────────────────────
...
class EMACrossover1Min:
    ...
    def _update_ema_crossover(self, rider: AlphaBearPut) -> None:
        ...
        st_bearish = st_dir == -1
        log_and_print(f"SUPERTREND(10,3) | 5-min direction={st_dir} | bearish={st_bearish}")

        # ── BEARISH SIGNAL CHECK ─────────────────────────────────────────────
        # Case 1: EMA9 crosses below EMA21 AND SuperTrend is bearish
        crossover_condition = bool(
            prev["EMA9"] >= prev["EMA21"] and latest["EMA9"] < latest["EMA21"] and st_bearish
        )
        # Case 2: Opening alignment — EMA9 already below EMA21 at first valid window
        opening_condition = False
        if not self.opening_checked:
            self.opening_checked = True
            if latest["EMA9"] < latest["EMA21"] and st_bearish:
                opening_condition = True
                log_and_print(
                    f"✅ OPENING ALIGNMENT | EMA9 already below EMA21, SuperTrend bearish | "
        ...
        log_and_print(
            ...
            f"ST_bearish={st_bearish} | crossover={crossover_condition} | signal={signal_condition}"
        )

        if signal_condition and self.last_trade_signal != 1:
            log_and_print(
                f"✅ BEARISH ENTRY SIGNAL | EMA9×EMA21 + SuperTrend bearish | "
                f"EMA9={latest['EMA9']:.2f} EMA21={latest['EMA21']:.2f} | "
...
def main():
    ...
    while True:
        ...
        # threading.Event keeps the session alive until the strategy fully finishes.
        done_event = threading.Event()
        ...
        try:
            ...
            while not done_event.is_set():
                if is_after_market_close_ist():
                    log_and_print("MAIN MARKET CLOSE WATCHDOG | Force closing bear put strategy.")
