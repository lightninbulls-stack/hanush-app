...
def rank_snapshot(
    ...
    """
    ascending=True -> lowest volatility gets rank 1
    """
    if rank_key not in snapshot.columns:
        raise ValueError(f"rank_key must be one of {list(snapshot.columns)}")

    ranked = snapshot.copy()
    ranked["rank"] = ranked[rank_key].rank(ascending=ascending, method="dense").astype(int)
    ranked = ranked.sort_values(["rank", rank_key], ascending=[True, ascending])
    return ranked
...
def minmax_score_low_vol(series: pd.Series) -> pd.Series:
    """
    ...
    if smax > smin:
        score = (1.0 - (s - smin) / (smax - smin)) * 100.0
...
def build_topn_ui_table(
    ...
    """
    Merge ranked low-vol output with the universe workbook metadata.
    ...
    merged["Score"] = minmax_score_low_vol(merged["vol_6m"])
