...
def fetch_from_value_csv() -> List[Dict]:
    base_dir = os.path.dirname(os.path.abspath(__file__))
    csv_path = os.path.join(base_dir, "..", "data", "value_latest.csv")

    try:
        if not os.path.exists(csv_path):
            print(f"Value CSV not found at {csv_path}")
            return []

        df = pd.read_csv(csv_path)
        print(f"Successfully read value CSV. Found {len(df)} rows.")
        ...
        print(f"Mapped {len(stocks)} value CSV records")
        return stocks

    except Exception as e:
        print(f"Error reading value CSV {csv_path}: {e}")
        return []
