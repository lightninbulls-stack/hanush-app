...
def fetch_from_regime_csv(category: str) -> List[Dict]:
    base_dir = os.path.dirname(os.path.abspath(__file__))

    file_map = {
        "Regime Upside": os.path.join(base_dir, "..", "data", "regime_upside_latest.csv"),
        "Regime Downside": os.path.join(base_dir, "..", "data", "regime_downside_latest.csv"),
    }

    csv_path = file_map.get(category)
    if not csv_path:
        print(f"No regime CSV configured for category: {category}")
        return []

    try:
        if not os.path.exists(csv_path):
            print(f"Regime CSV not found at {csv_path}")
            return []
        ...
        print(f"Error reading regime CSV {csv_path}: {e}")
        return []
