...
def fetch_from_range_bound_csv(category: str) -> List[Dict]:
    base_dir = os.path.dirname(os.path.abspath(__file__))

    file_map = {
        "Range Bound Upside": os.path.join(base_dir, "..", "data", "range_bound_upside_latest.csv"),
        "Range Bound Downside": os.path.join(base_dir, "..", "data", "range_bound_downside_latest.csv"),
    }

    csv_path = file_map.get(category)
    if not csv_path:
        print(f"No range bound CSV configured for category: {category}")
        return []

    try:
        if not os.path.exists(csv_path):
            print(f"Range Bound CSV not found at {csv_path}")
            return []

        df = pd.read_csv(csv_path)
        print(f"Successfully read {category} CSV. Found {len(df)} rows.")
        ...
        print(f"Mapped {len(stocks)} {category} records")
        return stocks

    except Exception as e:
        print(f"Error reading range bound CSV {csv_path}: {e}")
        return []
