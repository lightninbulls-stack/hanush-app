...
def load_value_input(
    value_input_path: Path,
    universe_meta: pd.DataFrame,
) -> pd.DataFrame:
    if not value_input_path.exists():
        raise FileNotFoundError(f"value input file not found: {value_input_path}")

    if value_input_path.suffix.lower() == ".csv":
        df = pd.read_csv(value_input_path)
    else:
        df = pd.read_excel(value_input_path)

    if df.empty:
        raise RuntimeError("value input file is empty.")
    ...
    symbol_col = next(
        (cols[c] for c in ["symbol", "nse symbol"] if c in cols),
        None,
    )
    ticker_col = next(
        (cols[c] for c in ["ticker", "yahoo finance ticker"] if c in cols),
        None,
    )
    company_col = next(
        (cols[c] for c in ["company name", "company", "underlying"] if c in cols),
        None,
    )
    ...
    if symbol_col is not None:
        ...
        selected = df.merge(
            universe_meta[
                ["Ticker", "NSE Symbol", "Sector", "Company", "symbol_key"]
            ],
            on="symbol_key",
            how="left",
        )
        ...
        selected = df.merge(
            universe_meta[
                ["Ticker", "NSE Symbol", "Sector", "Company", "ticker_key"]
            ],
            on="ticker_key",
            how="left",
        )
        ...
        selected = df.merge(
            universe_meta[
                ["Ticker", "NSE Symbol", "Sector", "Company", "company_key"]
            ],
            on="company_key",
            how="left",
        )
        ...
        raise ValueError(
            "value input must contain one of these columns: "
            "Symbol / NSE Symbol / Ticker / Company Name"
        )
    ...
    if not missing.empty:
        print("Warning: some value names/symbols could not be matched:")
...
def build_value_output(
    selected: pd.DataFrame,
    close: pd.DataFrame,
    asof_date: str,
) -> pd.DataFrame:
    tickers = [t for t in selected["Ticker"].tolist() if t in close.columns]

    if not tickers:
        raise RuntimeError("No matched value tickers found in close_prices_wide.csv.")

    close_subset = close[tickers].copy()

    ret_1w = latest_return(close_subset, TRADING_DAYS_1W).rename("ret_1w")
    ret_1m = latest_return(close_subset, TRADING_DAYS_1M).rename("ret_1m")
    ret_3m = latest_return(close_subset, TRADING_DAYS_3M).rename("ret_3m")
    ret_6m = latest_return(close_subset, TRADING_DAYS_6M).rename("ret_6m")
    vol_6m = latest_vol_6m(close_subset).rename("vol_6m")

    metrics = pd.concat([ret_1w, ret_1m, ret_3m, ret_6m, vol_6m], axis=1).reset_index()
    metrics = metrics.rename(columns={"index": "Ticker"})
    ...
    out = pd.DataFrame(
        {
            ...
            "Notes": merged["Notes"].fillna("Value Factor").astype(str),
        }
    )
...
def main() -> None:
    print("=" * 80)
    print("STARTING VALUE UPDATE FROM STORED CLOSE DATA")
    print("=" * 80)
    ...
    value_input_path = data_dir / "value_input.csv"
    output_csv_path = data_dir / "value_latest.csv"
    ...
    selected = load_value_input(value_input_path, universe_meta)
    print(f"Value input rows matched: {len(selected)}")
    ...
    value_ui.to_csv(output_csv_path, index=False)

    print("-" * 80)
    print(f"As-of date: {asof_date}")
    print(f"Rows exported: {len(value_ui)}")
    print(f"Output written to: {output_csv_path}")
    print("-" * 80)
    print(value_ui.head(10).to_string(index=False))
    print("=" * 80)
    print("VALUE UPDATE COMPLETED SUCCESSFULLY")
    print("=" * 80)
