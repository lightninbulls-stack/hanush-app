...
from api_service import tts_routes, signal_routes, partner_routes
...
try:
    ...
    with engine.connect() as _conn:
        ...
        _conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS referred_by_code VARCHAR"))
        _conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS referred_by_id INTEGER"))
        _conn.commit()
except Exception as _e:
    logger.warning("user column migration skipped: %s", _e)
