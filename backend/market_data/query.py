...
async def get_latest_price(db: AsyncSession, symbol: str) -> Optional[Dict]:
    for tf in ["1min", "5min", "1day"]:
        ...
        stmt = select(Model).where(Model.symbol == symbol).order_by(Model.timestamp.desc()).limit(1)
        result = await db.execute(stmt)
...
async def get_multi_symbol_latest(db: AsyncSession, symbols: List[str]) -> Dict[str, Dict]:
...
async def get_available_range(db: AsyncSession, symbol: str, timeframe: str) -> Optional[Tuple[datetime, datetime]]:
    Model = TIMEFRAME_MODEL_MAP.get(timeframe)
    if not Model:
        return None
    stmt = select(func.min(Model.timestamp), func.max(Model.timestamp)).where(Model.symbol == symbol)
    result = await db.execute(stmt)
