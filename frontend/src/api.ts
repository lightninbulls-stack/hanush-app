...
const SECTORAL_INDICES_CATEGORY = "Sectoral Indices Performance";
...
export function getCachedStocksByCategory(
  ...
  if (category === NSE_TOP_200_FO_CATEGORY || category === SECTORAL_INDICES_CATEGORY) {
...
export async function fetchStocksByCategory(
  ...
  if (category === SECTORAL_INDICES_CATEGORY) {
    window.localStorage.removeItem(getCacheKey(category));
    const response = await axios.get(
