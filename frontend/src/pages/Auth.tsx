...
const features: FeatureInfo[] = [
  ...
  },
  {
    ...
    icon: "◆",
    tag: "LIVE",
    what: "Upside Trend Stocks is a fully automated intraday signal engine that monitors a curated universe of NSE stocks in real time and enters a position the moment a stock shows confirmed short-term buying momentum overtaking its medium-term baseline — but only when the overall market is itself trending upward, eliminating counter-trend entries entirely.",
    model: "Every morning the engine pre-loads a filtered universe of stocks that are already in a positive market regime — meaning only stocks where the broader price environment supports upside moves. From market open, the system fetches live prices continuously for every stock in the universe. Before entering any position, the engine applies two independent checks: first, it confirms that the Nifty 50 index itself is in an upward trend at that moment — if the market is falling, no new buys are taken regardless of how good an individual stock looks; second, it checks that the individual stock's short-term buying pressure has cleanly and decisively crossed above its medium-term trend baseline by a meaningful margin, not just by a tiny tick. Only when both conditions are met simultaneously does the engine enter the trade. Up to 10 stocks can be active at any time, with each stock allocated equal capital of ₹50,000 buying power. Every slot is permanent — once a stock enters and exits for any reason, that slot is closed for the day and is not replaced by another stock. Each stock carries its own individual profit target of +1% and stop loss of -1.5%. At the portfolio level, if the total P&L across all positions reaches +2.5% of total capital the engine exits all remaining positions and locks in the day's gains. If the total P&L reaches -2.5% of total capital the engine exits all positions and stops trading for the day to protect capital. The strategy runs continuously from 9:15 AM to 3:15 PM and automatically restarts fresh the next trading day.",
    why: "Intraday trading without a system forces the trader to manually watch hundreds of stocks simultaneously, make split-second decisions under emotional pressure, and manage multiple positions by hand — all of which introduce inconsistency, fatigue, and costly errors. The biggest trap in momentum trading is entering a stock that looks bullish in isolation while the broader market is actually falling — these trades fail almost every time. Upside Trend Stocks solves this with a dual-confirmation engine: the market must agree with the stock signal before any trade is taken. The trader does not need to watch the screen. The engine handles scanning, confirmation, entry, exit, and risk management entirely within pre-defined rules, ensuring every decision is systematic, consistent, and free from emotion. The portfolio-level stops ensure that no single bad day can cause outsized damage to the overall capital.",
    example: "At 9:52 AM, the engine first checks that Nifty 50 is trending upward — it is. It then scans the stock universe and finds that ADANIENT's short-term buying pressure has decisively crossed above its medium-term trend baseline by a confirmed margin. Both conditions pass. The system automatically allocates ₹50,000 buying power, calculates the exact quantity, records the entry price, and begins monitoring. The stock hits its +1% target at 10:18 AM. The slot is permanently closed. The engine continues monitoring the remaining nine slots and running the portfolio-level risk checks until 3:15 PM.",
  },
  {
    name: "Downside Trend Stocks",
    icon: "◉",
    tag: "LIVE",
    what: "Downside Trend Stocks is a fully automated intraday signal engine that monitors a curated universe of NSE stocks in real time and enters a short position the moment a stock shows confirmed short-term selling pressure overtaking its medium-term baseline — but only when the overall market is itself trending downward, eliminating counter-trend short entries entirely.",
    model: "Every morning the engine pre-loads a filtered universe of stocks that are already in a negative market regime — meaning only stocks where the broader price environment supports downside moves. From market open, the system fetches live prices continuously for every stock in the universe. Before entering any position, the engine applies two independent checks: first, it confirms that the Nifty 50 index itself is in a downward trend at that moment — if the market is rising, no new short positions are taken regardless of how weak an individual stock looks; second, it checks that the individual stock's short-term selling pressure has cleanly and decisively crossed below its medium-term trend baseline by a meaningful margin, not just by a tiny tick. Only when both conditions are met simultaneously does the engine enter the trade. Up to 10 stocks can be active at any time, with each stock allocated equal capital of ₹50,000 buying power. Every slot is permanent — once a stock enters and exits for any reason, that slot is closed for the day and is not replaced by another stock. Each stock carries its own individual profit target of +1% (stock falls 1%) and stop loss of -1.5% (stock rises 1.5% against the position). At the portfolio level, if the total P&L across all positions reaches +2.5% of total capital the engine exits all remaining positions and locks in the day's gains. If the total P&L reaches -2.5% of total capital the engine exits all positions and stops trading for the day to protect capital. The strategy runs continuously from 9:15 AM to 3:15 PM and automatically restarts fresh the next trading day.",
    why: "Short selling is one of the hardest things to do in intraday trading because stocks that look weak in isolation often bounce sharply when the broader market reverses upward — these counter-trend shorts are the most common cause of losses for short traders. Downside Trend Stocks solves this with a dual-confirmation engine: the market must itself be declining before any short trade is taken. This eliminates the mistake of shorting a weak stock in a rising market entirely. The engine automates the entire pipeline — scanning, confirmation, entry, exit, and risk management — so the trader benefits from disciplined short-side exposure without the stress of manual monitoring or the danger of fighting the market. The portfolio-level stops ensure that a bad trending day on the short side cannot inflict uncontrolled damage to overall capital.",
    example: "At 10:04 AM, the engine first checks that Nifty 50 is trending downward — it is. It then scans the stock universe and finds that CROMPTON's short-term selling pressure has decisively crossed below its medium-term trend baseline by a confirmed margin. Both conditions pass. The system automatically allocates ₹50,000 buying power on the short side, records the entry price, and begins monitoring. If the stock drops 1% it exits with a profit. If it rises 1.5% against the position it exits with a controlled stop loss. The slot is permanently closed. The engine continues running the remaining nine slots and portfolio checks until 3:15 PM.",
  },
];
