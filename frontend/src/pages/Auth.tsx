...
const features: FeatureInfo[] = [
  ...
  },
  {
    ...
    icon: "◆",
    tag: "LIVE",
    what: "Upside Trend Stocks is a fully automated intraday signal engine that monitors a curated universe of NSE stocks in real time and alerts the moment a stock shows confirmed short-term buying momentum overtaking its medium-term baseline — signalling a high-probability long opportunity.",
    model: "Every morning the engine pre-loads a filtered universe of stocks that are already in a positive market regime — meaning only stocks where the broader price environment supports upside moves. From market open, the system fetches live prices every half second for every stock in the universe. It continuously measures short-term buying pressure against the medium-term price trend. The moment short-term momentum cleanly crosses above the medium-term trend for a given stock, the system registers a signal and enters the position automatically. Up to 10 stocks can be active at any time, with each stock allocated equal capital of ₹50,000 buying power. Every slot is permanent — once a stock enters and exits for any reason, that slot is closed for the day and is not replaced by another stock. Each stock carries its own individual profit target of +1% and stop loss of -1.5%. At the portfolio level, if the combined positions are up +2.5% on average the engine exits all remaining positions and locks in the day's gains. If the combined positions are down -2.5% on average the engine exits all positions and stops trading for the day to protect capital. The strategy runs continuously from 9:15 AM to 3:30 PM and automatically restarts fresh the next trading day.",
    why: "Intraday trading without a system forces the trader to manually watch hundreds of stocks simultaneously, make split-second decisions under emotional pressure, and manage multiple positions by hand — all of which introduce inconsistency, fatigue, and costly errors. Upside Trend Stocks removes every one of these problems by automating the scanning, the entry, the exit, and the risk management entirely. The trader does not need to watch the screen. The engine handles everything within pre-defined rules, ensuring every decision is systematic, consistent, and free from emotion. The portfolio-level stops ensure that no single bad day can cause outsized damage to the overall capital.",
    example: "At 9:52 AM, ADANIENT's short-term buying momentum crosses above its medium-term trend baseline — confirmed by the engine as a genuine momentum shift rather than a temporary price spike. The system automatically allocates ₹50,000 buying power, calculates the exact quantity, records the entry price, and begins monitoring. The stock hits its +1% target at 10:18 AM. The slot is permanently closed. The engine continues monitoring the remaining nine slots and running the portfolio-level risk checks until 3:30 PM.",
  },
  {
    name: "Downside Trend Stocks",
    icon: "◉",
    tag: "LIVE",
    what: "Downside Trend Stocks is a fully automated intraday signal engine that monitors a curated universe of NSE stocks in real time and alerts the moment a stock shows confirmed short-term selling pressure overtaking its medium-term baseline — signalling a high-probability short opportunity.",
    model: "Every morning the engine pre-loads a filtered universe of stocks that are already in a negative market regime — meaning only stocks where the broader price environment supports downside moves. From market open, the system fetches live prices every half second for every stock in the universe. It continuously measures short-term selling pressure against the medium-term price trend. The moment short-term weakness cleanly crosses below the medium-term trend for a given stock, the system registers a signal and enters the position automatically. Up to 10 stocks can be active at any time, with each stock allocated equal capital of ₹50,000 buying power. Every slot is permanent — once a stock enters and exits for any reason, that slot is closed for the day and is not replaced by another stock. Each stock carries its own individual profit target of +1% and stop loss of -1.5%. At the portfolio level, if the combined positions are up +2.5% on average the engine exits all remaining positions and locks in the day's gains. If the combined positions are down -2.5% on average the engine exits all positions and stops trading for the day to protect capital. The strategy runs continuously from 9:15 AM to 3:30 PM and automatically restarts fresh the next trading day.",
    why: "Identifying genuine intraday selling weakness versus temporary noise-driven dips requires a systematic multi-point confirmation process that no human can execute consistently across an entire universe of stocks. Downside Trend Stocks automates the entire pipeline — scanning, signal confirmation, entry, exit, and risk management — so the trader benefits from disciplined short-side exposure without the stress of manual monitoring. The portfolio-level stops ensure that a bad trending day on the short side cannot inflict uncontrolled damage to overall capital.",
    example: "At 10:04 AM, CROMPTON's short-term selling momentum crosses below its medium-term trend baseline — confirmed by the engine as a genuine bearish momentum shift. The system automatically allocates ₹50,000 buying power on the short side, records the entry price, and begins monitoring. If the stock drops -1% it exits with a profit. If it rises -1.5% against the position it exits with a controlled stop loss. The slot is permanently closed. The engine continues running the remaining nine slots and portfolio checks until 3:30 PM.",
  },
];
